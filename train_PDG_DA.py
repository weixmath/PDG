...
def train(args, io):
    ...
    source_train_loader = DataLoader(myBatchPaddingData_DA(pc_input_num=args.num_points, status='train', aug=True, swapax=swapax_src,
                                                pc_root='data/data_DA/'+args.src_dataset,sample_num=1,batch_size=1), num_workers=12,
                                    batch_size=32, shuffle=True, drop_last=True, pin_memory=True)
    target_test_loader = DataLoader(PaddingData(pc_input_num=args.num_points, status='test', aug=False, swapax=swapax_trg,
                                                pc_root='data/data_DA/'+args.trg_dataset), num_workers=4,
                                    batch_size=32, shuffle=True, drop_last=False, pin_memory=True)
    target_test_loader2 = DataLoader(PaddingData(pc_input_num=args.num_points, status='test', aug=False, swapax=swapax_trg2,
                                                pc_root='data/data_DA/' + args.trg_dataset2), num_workers=4,
                                    batch_size=32, shuffle=True, drop_last=False, pin_memory=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--src_dataset', type=str, default='modelnet', metavar='N',
                        choices=['modelnet,shapenet,scannet'])
    parser.add_argument('--trg_dataset', type=str, default='shapenet', metavar='N',
                        choices=['modelnet,shapenet,scannet'])
    parser.add_argument('--trg_dataset2', type=str, default='scannet', metavar='N',
                        choices=['modelnet,shapenet,scannet'])
    ...
    io.cprint(str(args))
    num_class = 10
    swapax_src = args.src_dataset == 'modelnet'
    swapax_trg = args.trg_dataset == 'modelnet'
    swapax_trg2 = args.trg_dataset2 == 'modelnet'
