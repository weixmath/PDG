...
def p_scan_DG(pc, pixel_size=0.017):
    pixel = int(2 / pixel_size)
    rotated_pc = rotate_point_cloud_3d(pc)
    pc_compress = (rotated_pc[:,2] + 1) / 2 * pixel  + (rotated_pc[:,1] + 1) / 2 * pixel
    ...
    # points_list = points_list[points_list!=None]
    # points_list = np.asarray(points_list,dtype=int)
    points_list = pc[points_list]
    return points_list
def p_scan_DA(pc, pixel_size=0.017):
    pixel = int(2 / pixel_size)
    rotated_pc = rotate_point_cloud_3d(pc)
